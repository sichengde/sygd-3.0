...
App.factory('dataService', ['webService', 'util', '$q', function (webService, util, $q) {
    ...
    var roomStateList = ["可用房", "团队房", "散客房", "走客房", "维修房","自用房","备用房"];//房间状态列表
     ...
     * 接口地址定义
     */
    var interfaceDoorList;
    function refreshInterfaceDoorList(p) {
        p = p || {};
        return webService.post('interfaceDoorGet', p)
            .then(function (d) {
                interfaceDoorList = d;
                return d;
            })
    ...
    function getInterfaceDoorList() {
        return interfaceDoorList;
    }
