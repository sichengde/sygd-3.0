...
public interface BookRoomCategoryMapper extends MyMapper<BookRoomCategory> {
     ...
     * 查找时间段内冲突的订单-返回房类和数量
     */
    @Select("SELECT count(*) num,room_category roomCategory FROM book_room LEFT JOIN book ON book.book_serial = book_room.book_serial LEFT JOIN room ON book_room.room_id = room.room_id WHERE date_format(book.reach_time , '%Y-%m-%d')< date_format(#{endTime}, '%Y-%m-%d') AND date_format(book.leave_time ,'%Y-%m-%d')>date_format(#{beginTime}, '%Y-%m-%d') GROUP BY room_category UNION SELECT count(*) num,room_category roomCategory FROM book_room_category LEFT JOIN book ON book.book_serial = book_room_category.book_serial WHERE date_format(book.reach_time , '%Y-%m-%d')< date_format(#{endTime}, '%Y-%m-%d') AND date_format(book.leave_time ,'%Y-%m-%d')>date_format(#{beginTime}, '%Y-%m-%d') GROUP BY room_category")
