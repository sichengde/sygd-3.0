...
public class DebtHistorySql {
    ...
    public String getTotalCancelDeposit(Map<String, Object> parameters) {
        ...
        String basic = "SELECT round(ifnull(sum(debt_history.deposit),0), 2) deposit FROM debt_history LEFT JOIN (SELECT DISTINCT pay_serial,user_id from debt_pay) dp ON debt_history.pay_serial = dp.pay_serial WHERE debt_history.done_time > #{beginTime} AND debt_history.done_time < #{endTime} AND ifnull(back, FALSE) = FALSE and ifnull(not_part_in,false)=false";
        if (userId != null) {
